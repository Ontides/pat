...
struct question {
    ...
    int wrongPersonCount = 0;//答错人数
    int index;//题目编号
    char correctOption[5];
};
bool cmp(question a, question b) {
    return a.wrongPersonCount > b.wrongPersonCount;
}
int main(){
    int n, m;
    scanf("%d %d", &n, &m);
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            ...
            bool correct = true;//当前题目是否回答正确
            if (selectedCount == q[j].correctOptionCount) {
                for (int k = 0; k < selectedCount; k++) {
                    if (selectedAnswer[k] != q[j].correctOption[k]) {
                        correct = false;
                        break;
                    }
                }
            ...
            //根据该题是否回答正确做相应处理
            if (correct) {
                score += q[j].fullScore;//增加该学生分数
            } else {
                q[j].wrongPersonCount++;//增加回答错误人数
            }
            getchar();//吸收空格和换行符
        }
        printf("%d\n", score);
    ...
    //对各个题目根据错误人数排序
    sort(q, q + m, cmp);
    if (q[0].wrongPersonCount == 0) {
        printf("Too simple");
    } else {
        printf("%d %d", q[0].wrongPersonCount, q[0].index);
        for (int i = 1; q[i].wrongPersonCount == q[0].wrongPersonCount; i++) {
            printf(" %d", q[i].index);
        }
    }
    return 0;
